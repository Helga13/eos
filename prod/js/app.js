...
//		, autoplay: true
	...
	// height left_col on page contacts.html
		var blockHeight = $('.left_col_wrap').height();
		$('.left_col__inner-height').css('height',blockHeight - 88);
	// quantity
	(function () {
...
//        console.log( radio );
    ...
    $('.sidebar_left').removeAttr('id');
    $('.sidebar_right').removeAttr('id');
	}
	if($(window).innerWidth() <= 600) {
    $('.left_col_wrap').find('.left_col__inner').removeClass('left_col__inner-height');
	}
	// toggle sidebar
	
	$('.nav_btn').on('click', function(e){
		e.preventDefault();
		$('.toggle_wrap').addClass('visible');
		$('.overlay_nav').addClass('visible').parent().css({'overflow': 'hidden'});
	});
	...
	$('.media_close').on('click', function(e){
		e.preventDefault();
...
// sticky
(function () {
	var a = document.querySelector('#fixblock')
		, b = null
		, P = 0;
...
(function () {
	var a = document.querySelector('#fixblock1')
		, b = null
		, P = 0;
	window.addEventListener('scroll', Ascroll, false);
	document.body.addEventListener('scroll', Ascroll, false);
